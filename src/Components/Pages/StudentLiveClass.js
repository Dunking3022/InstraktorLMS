...
const StudentLiveClass = (props) => {
    const userData = JSON.parse(localStorage.getItem("userData"));
    const id = userData.group;
    ...
    useEffect(() => {
      ...
      axios
        .get(`https://instraktor-be-v2.vercel.app/video/meeting/${id}`)
        .then((res) => {
          if (res.status === 200) {
            ...
            const callProperties = { url: `${domain}${id}`, showLeaveButton: true,
            showFullscreenButton: true, };
            
            // document.getElementsByClassName("banner").style.display = "none";

            let call = Daily.wrap(iframe,callProperties);
            call.setUserName(userData.studentid+"_"+userData.name);
